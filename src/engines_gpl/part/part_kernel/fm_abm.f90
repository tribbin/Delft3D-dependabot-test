...
!        vol2_rp = vol2
        ...
        if (itime == tstart_user + dts) then
            ...
            if(abmmt == model_asian_carp_eggs) then    ! asian carp eggs model
            ...
            if(abmmt == model_asian_carp_eggs .and. abmsd == dev_asian_carp_egg) then
...
!                    conc)
        ...
        l_csv_now = .false.
        if (l_csv) then
            if (itime >= it_start_m2 .and. itime <= it_stop_m2 .and. mod(itime - it_start_m2, idt_csv) < idelt) then
                l_csv_now = .true.
...
!                        totdep = totdep + h0(m * ilayer + nosegl)
  ...
  !              if(zmodel) nlay = laybot(n, m)  NO ZMODEL YET        ! get location specific number of layers when zmodel
                ...
                if (nosubs >= 4 + nstage + 1) dtemp = wpart(4 + nstage + 1, ipart)       !Get sum of all temperatures encountered
                if (nosubs >= 4 + nstage + 2) ddepth = wpart(4 + nstage + 2, ipart)       !Get sum of all depths encountered
                if (nosubs >= 4 + nstage + 3) sdepth = wpart(4 + nstage + 3, ipart)       !Get averaged depth endured in stage
                ...
                if (nosubs >= 4 + nstage + 1) dtemp = dtemp + temp                                 !Add all temperatures encountered to generate the averaged temp later on
                if (nosubs >= 4 + nstage + 2) ddepth = ddepth + zdepth                              !Add all depths encoutered to generate the averaged depth later on
                if (nosubs >= 4 + nstage + 3) sdepth = (sdepth * stime + zdepth * delt) / (stime + delt)    !Averaged depth endured in stage (including this timestep)
                ...
                if (istage == 0 .and. (.not. chronrev)) then      !If the stage is 0 and if chronologically modelled
                    if (stime > -(stage + delt)) then    !and the time spend is more than required for stage 0 (including this time step).
                    ...
                    if(istage == 0 .and. chronrev .and. (.not. stime > -(stage + delt))) then  !If the stage is 0 and reversed chronologically modelled
...
!                    duration = astage(istage) + bstage(istage) * (1 - dev_factor)
                ...
                case(dev_asian_carp_egg)                                                     !Stage development 3
                    ...
                    elseif (istage == 2) then                                              ! stage development for larvae without GBI (stage 2)
                ...
                if(istage == int(selstage) .and. frelease .and. chronrev) then      !If the stage is 0 and reversed chronologically modelled
                ...
                if(.not. chronrev) then
                    if (duration < stime) then       !Go to the next stage when this duration is lower than the time spent in the stage
                    ...
                    if(0.0 > stime) then                           ! if next stage reached
                ...
                if (istage > 0) then                                               !If the particles are released
                    mort = mort1(istage) + fstage * (mort2(istage) - mort1(istage))          ! Determine the mortality coefficient based on the stage
                    if (abs(tcmort(istage)) > 1.e-20) then                          !If the temperature based mortality is greater than 0
                ...
                do istage_t = 1, nstage                                                 !Loop for number of stages
                    if (istage_t + 4 <= nosubs) then                                   !?If stage index + 4 is <= number of substances then? (+4 is reference to wpart storage)
                        if (istage_t == istage) then                                  !If stage index is equal to current stage
                ...
                if (l_csv_now) then                                                                                       !If csv output on position is activated
                    ...
                    if(dtemp < -40.0 .or. dtemp == 0.) then                                                                                  ! check if model without temperature
                ...
                if (nosubs >= 4 + nstage + 1) wpart(4 + nstage + 1, ipart) = dtemp                                               ! Store dtemp
                if (nosubs >= 4 + nstage + 2) wpart(4 + nstage + 2, ipart) = ddepth                                              ! Store ddepth
                if (nosubs >= 4 + nstage + 3) wpart(4 + nstage + 3, ipart) = sdepth                                              ! Store sdepth
