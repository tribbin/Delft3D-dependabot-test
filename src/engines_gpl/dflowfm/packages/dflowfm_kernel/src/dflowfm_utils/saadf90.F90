...
600   ierr = ii
   ...
   subroutine pgmres(n, im, rhs, sol, vv, eps, maxits, iout, aa, ja, ia, alu, jlu, ju, ierr)
      use precision_basics, only: dp
      use m_saadf, only: dnrm2XXX, daxpyXXX
      implicit none

      integer :: n, im, maxits, iout, ierr, ja(*), ia(n + 1), jlu(*), ju(n)
      real(dp) :: vv(:, :), rhs(n), sol(n), aa(*), alu(*), eps
!----------------------------------------------------------------------*
!                                                                      *
...
4     i = i + 1
      its = its + 1
      i1 = i + 1
      call lusol(n, vv(:, i), rhs, alu, jlu, ju, 30 * n)
      call amux(n, rhs, vv(:, i1), aa, ja, ia)
!-----------------------------------------
!     modified gram - schmidt...
!-----------------------------------------
      do j = 1, i
         t = ddot(n, vv(:, j), 1, vv(:, i1), 1)
         hh(j, i) = t
         call daxpy(n, -t, vv(:, j), 1, vv(:, i1), 1)
      end do
      t = dnrm2(n, vv(1, i1), 1)
      hh(i1, i) = t
      if (t == 0.0d0) goto 58
