...
contains
    ...
    subroutine integration_scheme_17(buffer, file_unit_list, file_name_list, action, dlwqd, gridps)
        ...
        real(kind=dp) :: tol !< relative tolerance [10^-3, 10^-10]
        ...
        integer(kind=int_wp) :: iscale !< row scaling of system of equations [0 = no, 1 =yes]
        integer(kind=int_wp) :: nopred
        integer(kind=int_wp) :: iter   !< maximum number of iterations [ > 0]
        integer(kind=int_wp) :: ioptpc !< preconditioner switch 
                                       !< [0 = none, 1 = GS (L), 2 = GS (U), 3 = SSOR]
        ...
        associate (a => buffer%rbuf, j => buffer%ibuf, c => buffer%chbuf)
            ...
            do isys = 1, nosys
                ...
                ! note that the array for cell dumps is not filled yet
    ...
    end subroutine integration_scheme_17
