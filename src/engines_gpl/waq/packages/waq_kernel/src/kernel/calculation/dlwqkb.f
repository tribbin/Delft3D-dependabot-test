...
!                           for integer(kind=int_wp) ::block functions
!
...
!     IDTIME  INTEGER(kind=int_wp) ::1     IN/OUT  Delta for this function
!     ITIME1  INTEGER(kind=int_wp) ::1     IN/OUT  Lower time in file
!     ITIME2  INTEGER(kind=int_wp) ::1     IN/OUT  Higher time in file
!     IARRA1  REAL(kind=real_wp) ::NFTOT    IN/OUT  record at lower time
!     IARRA2  REAL(kind=real_wp) ::NFTOT    IN/OUT  record at higher time
!     NFTOT   INTEGER(kind=int_wp) ::1     INPUT   record length
