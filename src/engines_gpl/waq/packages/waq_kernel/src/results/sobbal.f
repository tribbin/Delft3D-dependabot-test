...
!     ITSTOP  INTEGER(kind=int_wp) ::1     INPUT   Stop time of the simulation
!     IMSTRT  INTEGER(kind=int_wp) ::1     INPUT   Start time of the output
!     IMSTOP  INTEGER(kind=int_wp) ::1     INPUT   Stop time of the output
...
!     ASMASS  REAL(kind=real_wp) ::NOTOT,*  IN/OUT  Cummulative balance per dump area
!                                         1   = mass
!                                         2   = processes
!                                         3/4 = loads in/out
!                                         5/6 = transport in/out
!     NOFLUX  INTEGER(kind=int_wp) ::1     INPUT   Number of fluxes
!     FLXINT  REAL(kind=real_wp) ::*     IN/OUT  Integrated fluxes in dump areas
!     NDMPAR  INTEGER(kind=int_wp) ::1       INPUT   Number of dump areas
!     NTDMPQ  INTEGER(kind=int_wp) ::1       INPUT
...
!     DMPQ    REAL(kind=real_wp) ::NOSYS*NDMPQ*? INPUT   mass balance dumped exchange
!     NOBND   INTEGER(kind=int_wp) ::1       INPUT   number of boundaries
!     NOBTYP  INTEGER(kind=int_wp) ::1       INPUT   number of boundaries types
!     BNDTYP  CHAR*20  NOBTYP     INPUT   boundary types names
!     INBTYP  INTEGER(kind=int_wp) ::NOBND      INPUT   boundary type number (index in BNDTYP)
!     NOCONS  INTEGER(kind=int_wp) ::1       INPUT   number of constants
!     CONAME  CHAR*20  NOCONS     INPUT   constants names
!     CONS    REAL(kind=real_wp) ::NOCONS     INPUT   constants array
!     NOQ     INTEGER(kind=int_wp) ::1       INPUT   number of exchanges
!     IPOINT  INTEGER(kind=int_wp) ::4,NOQ     INPUT   pointer array
!     FLXNAM  INTEGER(kind=int_wp) ::NOFLUX     INPUT   flux names
!     INTOPT  INTEGER(kind=int_wp) ::1       INPUT   Integration and balance suboptions
!     VOLUME  REAL(kind=real_wp) ::NOSEG      INPUT   Volume
!     SURF    REAL(kind=real_wp) ::NOSEG      INPUT   horizontal surface area
!     DMPBAL  INTEGER(kind=int_wp) ::NDMPAR     INPUT   if dump area is included in balance
!     NOWST   INTEGER(kind=int_wp) ::1       INPUT   number of wasteloads
!     NOWTYP  INTEGER(kind=int_wp) ::1       INPUT   number of wasteload types
!     WSTTYP  CHAR*20  NOWTYP     INPUT   wasteload types names
!     IWASTE  INTEGER(kind=int_wp) ::NOWST      INPUT   segment number wasteloads
!     INWTYP  INTEGER(kind=int_wp) ::NOWST      INPUT   wasteload type number (index in WSTTYP)
!     WSTDMP  REAL(kind=real_wp) ::NOTOT,NOWST,2  I   accumulated wasteloads 1/2 in and out
!     ==================================================================
!
      use m_outhis
      use m_dmpval
...
!     ----    -----    ------     ------- -----------
!     NOSUM   INTEGER(kind=int_wp) ::1       Nr of sum parameters
!     SFACTO  REAL(kind=real_wp) ::NOSUM,NOTOT  Factor for substance in sum parameters
!     STOCHL  REAL(kind=real_wp) ::NOSUM,NOFLUX Local STOCHI for sum parameters
!     NOOUT   INTEGER(kind=int_wp) ::1       Nr of balance terms per dump segment
!     IMASSA  INTEGER(kind=int_wp) ::NOTOT+NOSUM Pointer to accumulation term in balance
!     IEMISS  INTEGER(kind=int_wp) ::NOTOT+NOSUM Pointer to boundary terms in balance
!     NEMISS  INTEGER(kind=int_wp) ::1       Nr of boundary terms in balance
!     ITRANS  INTEGER(kind=int_wp) ::NOTOT+NOSUM Pointer to int. transport terms in balance
!     IPROCS  INTEGER(kind=int_wp) ::NOTOT+NOSUM Pointer to processes term(s) in balance
!     NPROCS  INTEGER(kind=int_wp) ::NOTOT+NOSUM Nr of processes terms in balance
!     BALANS  REAL(kind=real_wp) ::NOOUT,*      Mass balances for current time
!     BALTOT  REAL(kind=real_wp) ::NOOUT,*      Integrated mass balances
...
!     IBSTRT  INTEGER(kind=int_wp) ::1       Proper start time of the balance period
!     IBSTOP  INTEGER(kind=int_wp) ::1       Proper stop time of the balance period
