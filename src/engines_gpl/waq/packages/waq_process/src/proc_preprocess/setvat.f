...
!     Locals
!
      integer(kind=int_32), PARAMETER   ::MAXLOC = 2000
      INTEGER(kind=int_32) ::VATTAG(MAXLOC), VATTDA(MAXLOC)
     ...
     +             DENAME(*)
      ...
      LOGICAL      LEXI
      INTEGER(kind=int_32) ::LUN, IIVOL ,IIAREA, IIFLOW, IILENG, IIDISP, IICONC,
     ...
     +             IIBUX, IIWRK1, IIWRK2, IIAAKL, IIBBKL, IICCKL, IIDDKL
      ...
      integer(kind=int_32) ::IVVOL,IVARE,IVFLO,IVLEN,IVCNS,IVPAR,IVFUN,IVSFU,IVCNC,
     +             IVMAS,IVDER,IVDSP,IVVEL,IVDEF,IVLOC,IVDSX,IVVLX,IVLCX,
     +             IVFLX

      integer(kind=int_32) ::isys, ipa, ifun, idsp, isfun, ivel, idef, ivar, iv_da, iv_ag,
