...
C     from PMSA array
      ...
      REAL(kind=sp) ::TEMP               ! 1  in
      REAL(kind=sp) ::BIOMAS_MPB1        ! 2  in
      REAL(kind=sp) ::BIOMAS_MPB2        ! 3  in
      REAL(kind=sp) ::BIOMAS_S1_MPB1     ! 4  in
      REAL(kind=sp) ::BIOMAS_S1_MPB2     ! 5  in
      REAL(kind=sp) ::PMCH20_MPB1        ! 6  in
      REAL(kind=sp) ::PMCH20_MPB2        ! 7  in
      REAL(kind=sp) ::FLT_MPB1           ! 8  in
      REAL(kind=sp) ::FLT_MPB2           ! 9  in
      REAL(kind=sp) ::FTMP_MPB1          ! 10 in
      REAL(kind=sp) ::FTMP_MPB2          ! 11 in
      REAL(kind=sp) ::FNUT_MPB1          ! 12 in
      REAL(kind=sp) ::FNUT_MPB2          ! 13 in
      REAL(kind=sp) ::R_PR_MPB1          ! 14 in
      REAL(kind=sp) ::R_PR_MPB2          ! 15 in
      REAL(kind=sp) ::R_MT20_MPB1        ! 16 in
      REAL(kind=sp) ::R_MT20_MPB2        ! 17 in
      REAL(kind=sp) ::RT_MPB1            ! 18 in
      REAL(kind=sp) ::RT_MPB2            ! 19 in
      REAL(kind=sp) ::B_EX_MPB1          ! 20 in
      REAL(kind=sp) ::B_EX_MPB2          ! 21 in
      REAL(kind=sp) ::M1_20_MPB1         ! 22 in
      REAL(kind=sp) ::M1_20_MPB2         ! 23 in
      REAL(kind=sp) ::M2_20_MPB1         ! 24 in
      REAL(kind=sp) ::M2_20_MPB2         ! 25 in
      REAL(kind=sp) ::MT_MPB1            ! 26 in
      REAL(kind=sp) ::MT_MPB2            ! 27 in
      REAL(kind=sp) ::NCRAT_MPB1         ! 28 in
      REAL(kind=sp) ::NCRAT_MPB2         ! 29 in
      REAL(kind=sp) ::PCRAT_MPB1         ! 30 in
      REAL(kind=sp) ::PCRAT_MPB2         ! 31 in
      REAL(kind=sp) ::SCRAT_MPB1         ! 32 in
      REAL(kind=sp) ::SCRAT_MPB2         ! 33 in
      REAL(kind=sp) ::FAM_MPB1           ! 34 in
      REAL(kind=sp) ::FAM_MPB2           ! 35 in
      REAL(kind=sp) ::FNI_MPB1           ! 36 in
      REAL(kind=sp) ::FNI_MPB2           ! 37 in
      REAL(kind=sp) ::TRESH_MPB1         ! 38 in
      REAL(kind=sp) ::TRESH_MPB2         ! 39 in
      LOGICAL            :: S1_BOTTOM          ! 40 in  , switch for S1 bottom approach (.true.) or DELWAQ-G approach (.false.)
      REAL(kind=sp) ::FLT_S1_MPB1        ! 41 in
      REAL(kind=sp) ::FLT_S1_MPB2        ! 42 in
      REAL(kind=sp) ::FTMP_S1_MPB1       ! 43 in
      REAL(kind=sp) ::FTMP_S1_MPB2       ! 44 in
      REAL(kind=sp) ::FNUT_S1_MPB1       ! 45 in
      REAL(kind=sp) ::FNUT_S1_MPB2       ! 46 in
      REAL(kind=sp) ::FAM_S1_MPB1        ! 47 in
      REAL(kind=sp) ::FAM_S1_MPB2        ! 48 in
      REAL(kind=sp) ::FNI_S1_MPB1        ! 49 in
      REAL(kind=sp) ::FNI_S1_MPB2        ! 50 in
      REAL(kind=sp) ::NH4                ! 51 in
      REAL(kind=sp) ::NO3                ! 52 in
      REAL(kind=sp) ::PO4                ! 53 in
      REAL(kind=sp) ::SI                 ! 54 in
      REAL(kind=sp) ::ZSED               ! 55 in
      REAL(kind=sp) ::SURF               ! 56 in
      REAL(kind=sp) ::DEPTH              ! 57 in
      REAL(kind=sp) ::DELT               ! 58 in
      REAL(kind=sp) ::dBotN              ! 59 in
      REAL(kind=sp) ::dSWN               ! 60 in
      REAL(kind=sp) ::dGSNH              ! 61 in
      REAL(kind=sp) ::dGSNO              ! 62 in
      REAL(kind=sp) ::dBotP              ! 63 in
      REAL(kind=sp) ::dSWP               ! 64 in
      REAL(kind=sp) ::dGSP               ! 65 in
      REAL(kind=sp) ::dBotSi             ! 66 in
      REAL(kind=sp) ::dSWSi              ! 67 in
      REAL(kind=sp) ::CCAP_MPB1          ! 68 in   carrying capacity MPB1                     (gC/m2)
      REAL(kind=sp) ::CCAP_MPB2          ! 69 in   carrying capacity MPB2                     (gC/m2)
      REAL(kind=sp) ::LOCSEDDEPT         ! 70 in   Sediment layer depth to bottom of segment      (m)
      REAL(kind=sp) ::OXY                ! 71 in   Dissolved Oxygen                            (g/m3)
      REAL(kind=sp) ::MPBOXYCRIT         ! 72 in   Crit. oxygen conc. for growth and resp. MPB (g/m3)
      REAL(kind=sp) ::MPB1MO_20          ! 73 in   MPB1peli mortality at 20°C under Oxygen depl.(1/d)
      REAL(kind=sp) ::MPB2MO_20          ! 74 in   MPB2psam mortality at 20°C under Oxygen depl.(1/d)
      REAL(kind=sp) ::BIOMAS_MPB1_M2     ! 75 out, MPB1peli biomass per m3 in layer S1        (gC/m3)
      REAL(kind=sp) ::BIOMAS_MPB2_M2     ! 76 out, MPB2psam biomass per m3 in layer S1        (gC/m3)
      REAL(kind=sp) ::BIOMAS_S1_MPB1_M3  ! 77 out, MPB1peli biomass per m3 in layer S1        (gC/m3)
      REAL(kind=sp) ::BIOMAS_S1_MPB2_M3  ! 78 out, MPB2psam biomass per m3 in layer S1        (gC/m3)
      REAL(kind=sp) ::MPB1FMC            ! 79 out, logistic growth restaint factor MPB1           (-)
      REAL(kind=sp) ::MPB2FMC            ! 80 out, logistic growth restaint factor MPB2           (-)
      REAL(kind=sp) ::MPB1FMN            ! 81 out, correction factor insufficient nitrogen MPB1   (-)
      REAL(kind=sp) ::MPB2FMN            ! 82 out, correction factor insufficient nitrogen MPB2   (-)
      REAL(kind=sp) ::MPB1FMP            ! 83 out, correction factor insufficient phosphorus MPB1 (-)
      REAL(kind=sp) ::MPB2FMP            ! 84 out, correction factor insufficient phosphorus MPB2 (-)
      REAL(kind=sp) ::MPB1FMS            ! 85 out, correction factor insufficient silicate MPB1   (-)
      REAL(kind=sp) ::MPB2FMS            ! 86 out, correction factor insufficient silicate MPB2   (-)
      REAL(kind=sp) ::MPB1FMCS1          ! 87 out, logistic growth restaint factor MPB1 in S1     (-)
      REAL(kind=sp) ::MPB2FMCS1          ! 88 out, logistic growth restaint factor MPB2 in S1     (-)
      REAL(kind=sp) ::MPB1FMNS1          ! 89 out, corr. factor insufficient nitrogen MPB1 in S1  (-)
      REAL(kind=sp) ::MPB2FMNS1          ! 90 out, corr. factor insufficient nitrogen MPB2 in S1  (-)
      REAL(kind=sp) ::MPB1FMPS1          ! 91 out, corr. factor insufficient phosphorus MPB1 in S1(-)
      REAL(kind=sp) ::MPB2FMPS1          ! 92 out, corr. factor insufficient phosphorus MPB2 in S1(-)
      REAL(kind=sp) ::MPB1FMSS1          ! 93 out, corr. factor insufficient silicate MPB1 in S1  (-)
      REAL(kind=sp) ::MPB2FMSS1          ! 94 out, corr. factor insufficient silicate MPB2 in S1  (-)
      REAL(kind=sp) ::FMPB1NH4UP         ! 95 out, MPB1 net consumption rate for ammonium   (gN/m3/d)
      REAL(kind=sp) ::FMPB2NH4UP         ! 96 out, MPB2 net consumption rate for ammonium   (gN/m3/d)
      REAL(kind=sp) ::FMPB1NO3UP         ! 97 out, MPB1 net consumption rate for nitrate    (gN/m3/d)
      REAL(kind=sp) ::FMPB2NO3UP         ! 98 out, MPB2 net consumption rate for nitrate    (gN/m3/d)
      REAL(kind=sp) ::FMPB1PO4UP         ! 99 out, MPB1 net consumption rate for phosphate  (gP/m3/d)
      REAL(kind=sp) ::FMPB2PO4UP         !100 out, MPB2 net consumption rate for phosphate  (gP/m3/d)
      REAL(kind=sp) ::FMPB1SIUP          !101 out, MPB1 net consumption rate for silicate  (gSi/m3/d)
      REAL(kind=sp) ::FMPB2SIUP          !102 out, MPB2 net consumption rate for silicate  (gSi/m3/d)
      REAL(kind=sp) ::FMPB1EXC           !103 out, MPB1 excretion rate                      (gC/m3/d)
      REAL(kind=sp) ::FMPB2EXC           !104 out, MPB2 excretion rate                      (gC/m3/d)
      REAL(kind=sp) ::FMPB1FGP           !105 out, MPB1 gross primary production rate       (gC/m3/d)
      REAL(kind=sp) ::FMPB2FGP           !106 out, MPB2 gross primary production rate       (gC/m3/d)
      REAL(kind=sp) ::FMPB1MOR           !107 out, MPB1 total mortality rate                (gC/m3/d)
      REAL(kind=sp) ::FMPB2MOR           !108 out, MPB2 total mortality rate                (gC/m3/d)
      REAL(kind=sp) ::FMPB1POC1          !109 out, MPB1 net production rate for POC1        (gC/m3/d)
      REAL(kind=sp) ::FMPB2POC1          !110 out, MPB2 net production rate for POC1        (gC/m3/d)
      REAL(kind=sp) ::FMPB1PON1          !111 out, MPB1 net production rate for PON1        (gN/m3/d)
      REAL(kind=sp) ::FMPB2PON1          !112 out, MPB2 net production rate for PON1        (gN/m3/d)
      REAL(kind=sp) ::FMPB1POP1          !113 out, MPB1 net production rate for POP1        (gP/m3/d)
      REAL(kind=sp) ::FMPB2POP1          !114 out, MPB2 net production rate for POP1        (gP/m3/d)
      REAL(kind=sp) ::FMPB1OPAL          !115 out, MPB1 net production rate for OPAL Si    (gSi/m3/d)
      REAL(kind=sp) ::FMPB2OPAL          !116 out, MPB2 net production rate for OPAL Si    (gSi/m3/d)
      REAL(kind=sp) ::FMPB1OXY           !117 out, MPB1 net production rate for DO          (gO/m3/d)
      REAL(kind=sp) ::FMPB2OXY           !118 out, MPB2 net production rate for DO          (gO/m3/d)
      REAL(kind=sp) ::FMPB1RES           !119 out, MPB1 total respiration rate              (gC/m3/d)
      REAL(kind=sp) ::FMPB2RES           !120 out, MPB2 total respiration rate              (gC/m3/d)
      REAL(kind=sp) ::FMPB1FGPM2         !121 out, MPB1 gross primary production rate per m2(gC/m2/d)
      REAL(kind=sp) ::FMPB2FGPM2         !122 out, MPB2 gross primary production rate per m2(gC/m2/d)
      REAL(kind=sp) ::FMPB1FGPD          !123 out, MPB1 gross primary production rate per day   (1/d)
      REAL(kind=sp) ::FMPB2FGPD          !124 out, MPB2 gross primary production rate per day   (1/d)
      REAL(kind=sp) ::FMPB1NH4S1         !125 out, MPB1 net consumption rate for ammonium S1(gN/m3/d)
      REAL(kind=sp) ::FMPB2NH4S1         !126 out, MPB2 net consumption rate for ammonium S1(gN/m3/d)
      REAL(kind=sp) ::FMPB1NO3S1         !127 out, MPB1 net consumption rate for nitrate S1 (gN/m3/d)
      REAL(kind=sp) ::FMPB2NO3S1         !128 out, MPB2 net consumption rate for nitrate S1 (gN/m3/d)
      REAL(kind=sp) ::FMPB1PO4S1         !129 out, MPB1 net consumption rate for phosphateS1(gP/m3/d)
      REAL(kind=sp) ::FMPB2PO4S1         !130 out, MPB2 net consumption rate for phosphateS1(gP/m3/d)
      REAL(kind=sp) ::FMPB1SIS1          !131 out, MPB1 net consumption rate for silicateS1(gSi/m3/d)
      REAL(kind=sp) ::FMPB2SIS1          !132 out, MPB2 net consumption rate for silicateS1(gSi/m3/d)
      REAL(kind=sp) ::FMPB1EXCS1         !133 out, MPB1 excretion rate S1                   (gC/m3/d)
      REAL(kind=sp) ::FMPB2EXCS1         !134 out, MPB2 excretion rate S1                   (gC/m3/d)
      REAL(kind=sp) ::FMPB1FGPS1         !135 out, MPB1 gross primary production rate S1    (gC/m3/d)
      REAL(kind=sp) ::FMPB2FGPS1         !136 out, MPB2 gross primary production rate S1    (gC/m3/d)
      REAL(kind=sp) ::FMPB1MORS1         !137 out, MPB1 total mortality rate S1             (gC/m3/d)
      REAL(kind=sp) ::FMPB2MORS1         !138 out, MPB2 total mortality rate S1             (gC/m3/d)
      REAL(kind=sp) ::FMPB1POC1S         !139 out, MPB1 net production rate for POC1 S1     (gC/m3/d)
      REAL(kind=sp) ::FMPB2POC1S         !140 out, MPB2 net production rate for POC1 S1     (gC/m3/d)
      REAL(kind=sp) ::FMPB1PON1S         !141 out, MPB1 net production rate for PON1 S1     (gN/m3/d)
      REAL(kind=sp) ::FMPB2PON1S         !142 out, MPB2 net production rate for PON1 S1     (gN/m3/d)
      REAL(kind=sp) ::FMPB1POP1S         !143 out, MPB1 net production rate for POP1 S1     (gP/m3/d)
      REAL(kind=sp) ::FMPB2POP1S         !144 out, MPB2 net production rate for POP1 S1     (gP/m3/d)
      REAL(kind=sp) ::FMPB1OPALS         !145 out, MPB1 net production rate for OPAL Si S1 (gSi/m3/d)
      REAL(kind=sp) ::FMPB2OPALS         !146 out, MPB2 net production rate for OPAL Si S1 (gSi/m3/d)
      REAL(kind=sp) ::FMPB1OXYS1         !147 out, MPB1 net production rate for DO S1       (gO/m3/d)
      REAL(kind=sp) ::FMPB2OXYS1         !148 out, MPB2 net production rate for DO S1       (gO/m3/d)
      REAL(kind=sp) ::FMPB1RESS1         !149 out, MPB1 total respiration rate S1           (gC/m3/d)
      REAL(kind=sp) ::FMPB2RESS1         !150 out, MPB2 total respiration rate S1           (gC/m3/d)
      REAL(kind=sp) ::FMPB1GPS1M         !151 out, MPB1 gross primary production rate S1 m2 (gC/m2/d)
      REAL(kind=sp) ::FMPB2GPS1M         !152 out, MPB2 gross primary production rate S1 m2 (gC/m2/d)
      REAL(kind=sp) ::FMPB1GPS1D         !153 out, MPB1 gross primary production rate S1 per day(1/d)
      REAL(kind=sp) ::FMPB2GPS1D         !154 out, MPB2 gross primary production rate S1 per day(1/d)

C     local

      INTEGER(kind=int_32) ::ISEG               ! loop counter segment loop
      INTEGER(kind=int_32) ::IFLUX              ! index pointer in FL (flux) array
      ...
      INTEGER(kind=int_32), parameter  ::NO_POINTER = 154   ! number of input output variables in PMSA array
      INTEGER(kind=int_32) ::IP(NO_POINTER)     ! index pointer in PMSA array updated for each segment
      REAL(kind=sp) ::C_UPTAKE
      REAL(kind=sp) ::DMINN
      REAL(kind=sp) ::DMINP
      REAL(kind=sp) ::DMINS
      REAL(kind=sp) ::DN
      REAL(kind=sp) ::DP
      REAL(kind=sp) ::DSI
      REAL(kind=sp) ::FACTOR_MPB1
      REAL(kind=sp) ::FACTOR_MPB2
      REAL(kind=sp) ::FEXC_MPB1
      REAL(kind=sp) ::FEXC_MPB2
      REAL(kind=sp) ::FGP_MPB1
      REAL(kind=sp) ::FGP_MPB2
      REAL(kind=sp) ::FMOR_MPB1
      REAL(kind=sp) ::FMOR_MPB2
      REAL(kind=sp) ::FNH4_MPB1
      REAL(kind=sp) ::FNH4_MPB2
      REAL(kind=sp) ::FNO3_MPB1
      REAL(kind=sp) ::FNO3_MPB2
      REAL(kind=sp) ::FRES_MPB1
      REAL(kind=sp) ::FRES_MPB2
      REAL(kind=sp) ::FN_MPB1
      REAL(kind=sp) ::FN_MPB2
      REAL(kind=sp) ::NH4_UPTAKE
      REAL(kind=sp) ::NO3_UPTAKE
      REAL(kind=sp) ::N_UPTAKE
      REAL(kind=sp) ::OPAL_PROD
      REAL(kind=sp) ::PO4_UPTAKE
      REAL(kind=sp) ::POC_PROC
      REAL(kind=sp) ::PON_PROD
      REAL(kind=sp) ::POP_PROD
      REAL(kind=sp) ::P_UPTAKE
      REAL(kind=sp) ::SED2WAT
      REAL(kind=sp) ::SI_UPTAKE
      REAL(kind=sp) ::VOLSED             ! bulk volume of the sediment layer
      REAL(kind=sp) ::VOLWAT             ! volume
      REAL(kind=sp) ::MRES_MPB1          ! maintenance respiration MPB1
      REAL(kind=sp) ::MRES_MPB2          ! maintenance respiration MPB2
      REAL(kind=sp) ::FGP_MPB1_ORG       ! uncorrected FGP MPB1
      REAL(kind=sp) ::FGP_MPB2_ORG       ! uncorrected FGP MPB2
      REAL(kind=sp) ::EUF_FACT           ! correction factor for euphotic depth
      REAL(kind=sp) ::BIOMAS_MPB1_EUF    ! biomass corrected for euphotic depth
      REAL(kind=sp) ::BIOMAS_MPB2_EUF    ! biomass corrected for euphotic depth
