...
!
     ...
     &                   swblsolint, swblobject, bltemlim, blbasmor, swblgrochk, blbiobas, swblmorchk, bltoplev)
      ...
      integer(kind=int_32) ::ntyp_m               ! Maximum number of types
      integer(kind=int_32) ::ntyp_a               ! Actual number of types
      integer(kind=int_32) ::ngro_a               ! Actual number of groups
      integer(kind=int_32) ::nunucom              ! Number of constrains
      integer(kind=int_32) ::j, k, is             ! Indexes
...
!     Local variables
      character(120) idstr
      integer(kind=int_32) ::lparam, i, i1, i2
      real(kind=sp) ::autofr
      character(8)   cpmax
