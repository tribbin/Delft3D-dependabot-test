...
contains
    ...
    subroutine sedhm  (process_space_real, fl, ipoint, increm, num_cells, &
        ...
        do iseg = 1, num_cells
            if (btest(iknmrk(iseg), 0)) then
                ...
                if ((ikmrk2==0).or.(ikmrk2==3)) then
                    ...
                    fl4s2 = process_space_real(ipnt(9))
                    fl5s2 = process_space_real(ipnt(10))
                    q1 = process_space_real(ipnt(11))
                    q2 = process_space_real(ipnt(12))
                    q3 = process_space_real(ipnt(13))
                    q4 = process_space_real(ipnt(14))
                    q5 = process_space_real(ipnt(15))
                    depth = process_space_real(ipnt(16))
                    ...
                    process_space_real(ipnt(27)) = fl1 * q1 + fl2 * q2 + fl3 * q3 + fl4 * q4 + fl5 * q5
                    fl (1 + iflux) = process_space_real(ipnt(27)) / depth

                    process_space_real(ipnt(28)) = fl1s2 * q1 + fl2s2 * q2 + fl3s2 * q3 + fl4s2 * q4 + fl5s2 * q5
                    fl (2 + iflux) = process_space_real(ipnt(28)) / depth
        ...
        do iq = 1, num_exchanges_u_dir + num_exchanges_v_dir
            ...
            !........VxSedHM op nul
            process_space_real(ipnt(29)) = 0.0
            ipnt(29) = ipnt(29) + increm(29)
        ...
        ipnt(24) = ipnt(24) + (num_exchanges_u_dir + num_exchanges_v_dir) * increm(24)
        ipnt(25) = ipnt(25) + (num_exchanges_u_dir + num_exchanges_v_dir) * increm(25)
        ...
        do iq = num_exchanges_u_dir + num_exchanges_v_dir + 1, num_exchanges_u_dir + num_exchanges_v_dir + num_exchanges_z_dir
            ...
            if (ifrom > 0) then
                fhmim1 = process_space_real(ipnt(17) + (ifrom - 1) * increm(17))
                fhmim2 = process_space_real(ipnt(18) + (ifrom - 1) * increm(18))
                fhmim3 = process_space_real(ipnt(19) + (ifrom - 1) * increm(19))
                fhmpoc = process_space_real(ipnt(20) + (ifrom - 1) * increm(20))
                fhmphy = process_space_real(ipnt(21) + (ifrom - 1) * increm(21))
                vsim1 = process_space_real(ipnt(22))
                vsim2 = process_space_real(ipnt(23))
                vsim3 = process_space_real(ipnt(24))
                vspoc = process_space_real(ipnt(25))
                vsphy = process_space_real(ipnt(26))

                !...........Calculate VxSedHM
                process_space_real(ipnt(number_inp_out)) = fhmim1 * vsim1 + fhmim2 * vsim2 + fhmim3 * vsim3 + &
            ...
            ipnt(25) = ipnt(25) + increm(25)
            ipnt(26) = ipnt(26) + increm(26)
            ipnt(29) = ipnt(29) + increm(29)
