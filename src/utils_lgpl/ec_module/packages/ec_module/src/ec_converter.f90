...
contains
   ...
   function ecConverterSetElement(instancePtr, converterId, indx) result(success)
      ...
      type(tEcInstance), pointer            :: instancePtr !< intent(in)
      integer,                   intent(in) :: converterId !< unique Converter id
      integer,                   intent(in) :: indx        !< array index in target item array where to put the
      ! converted data. NOTE: call site should already account for the vectormax/n_data in this index.
