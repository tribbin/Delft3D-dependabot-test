...
\memoDate {\today}
\memoTelephone{\phantom{m} +31\,(0)88\,335\,8568 }
\memoEmail{\phantom{m} black-ops@deltares.nl}
\memoCopy{---}
...
\subsection{python}
\label{ssec:python}
The testbench for D-Hydro is written for python. The new testbench can be run with python and the necessary requirements installed within the requirements.txt.
...
\Note a common issue is "Activate.ps1 cannot be loaded because the execution of scripts is disabled on this system." Look here on how to troubleshoot: \autoref{sec:troubleshooting}

Now we need to fill this virtual environment with the required python packages. If the venv is successfully activated your terminal within vscode will show (venv) in front of the terminal. If the venv is successfully activated you can run the command \command{pip install -r requirements.txt} from within the deltares\_testbench folder.
...
\begin{guilist}
\item[\texttt{-{}-username}] Your username to download testcases.

\item[\texttt{-{}-password}] Your password to download testcases. It is recommended to use \command{-{}-interactive} to supply your password for running the testbench.
...
\begin{itemize}
\item{MinIO} and specifically the maintaining of the object repository by updating/adding references and cases.
...
To download from MinIO using the testbench you need to create an access key. The MinIO api only accepts access keys to login, your normal credentials don't work.
In the console there is a tab for access key: \autoref{fig:minioaccess}, you can also directly open the page using this link:\url{https://s3-console.deltares.nl/access-keys}.
\begin{figure}[H]
\centering
...
When you press create this will open a popup this will show your access key and secret key: \autoref{fig:minioaccesscreate}. Store these somewhere for example a password database like keepass.
\newline\textcolor{red}{This is the last time it will show you secret key if you lose it you need to create a new key.}
\begin{figure}[H]
\centering
...
\begin{itemize}
\item{open the website} and relevent folder to update or place new files under.
\item{upload} press the upload button: and choose either file or folder depending on what you want to update. \autoref{fig:minioupload}
...
To get correct teamcity logging in a way that teamcity understand which test we run and what the results, use the argument \command{-{}-teamcity}. Credentials need to be passed to access the object repository within teamcity, that is then passed with \command{-{}-username <username>} and \command{-{}-password <password>}.
...
\item["Activate.ps1 cannot be loaded because the execution of scripts is disabled on this system."] If the activate command for the python virtual enviroment generates the error that you are not allowed to execute a script, then you need to change the PowerShell execution policy to allow scripts to run. You do this by opening powershell as adminstrator and execute the command: \command{Set-ExecutionPolicy -ExecutionPolicy RemoteSigned}
