...
program tests_dlwq5a
   ...
   !!     Run unit tests for routines in Delft3D-WAQ
    ...
    call prepare_tests
    call runtests_init
    call runtests(tests_dlwq5a_all)
    call runtests_final
    ...
    subroutine tests_dlwq5a_all
       ...
       ! Run tests
       call test(test_dlwq5a_1_item_1_conc_1_const,   'One bc item, concentration of one substance and one data constant should be ok.')
       call test(test_dlwq5a_1_item_2_conc_2_const,   'One bc item, concentration of two substances and two respective data constants should be ok.')
       call test(test_dlwq5a_1_item_1_conc_1_tseries, 'One bc item, concentration of one substance and time series should be ok.')
       call test(test_dlwq5a_1_item_2_conc_1_tseries, 'One bc item, concentration of two substance given time series of one column (using USEFOR) should be ok.')

       ! Tear-down
       call teardown_all()
    end subroutine tests_dlwq5a_all
    ...
    subroutine test_dlwq5a_1_item_2_conc_1_tseries
        ...
        !< Test defining one bc item, and concentration of two substances employing USEFOR to be defined by a time series of one column.
        type(bc_fixture) :: fx       !< fixture containing all local variables
        ...
        call assert_equal(fx%car(3), 'OXY', 'Validate name of first substance in boundary condition.')
        call assert_equal(fx%car(4), 'Salinity', 'Validate name of second substance in boundary condition.')
        call assert_equal(fx%car(5), 'Salinity', 'Validate name of first substance in boundary condition after substitutions (USEFOR).')
        call assert_equal(fx%car(6), 'Salinity', 'Validate name of second substance in boundary condition after substitutions (USEFOR).')
        call assert_true(fx%rar(1) == 5.0, 'Validate value of boundary condition for first block in time series.')
        call assert_true(fx%rar(2) == 4.0, 'Validate value of boundary condition for second block in time series.')
        call assert_true(fx%rar(3) == 5.0, 'Validate value of boundary condition for first substance in first block of time series.')
        call assert_true(fx%rar(4) == 5.0, 'Validate value of boundary condition for second substance in first block of time series.')
        call assert_true(fx%rar(5) == 4.0, 'Validate value of boundary condition for first substance in second block of time series.')
        call assert_true(fx%rar(6) == 4.0, 'Validate value of boundary condition for second substance in second block of time series.')
